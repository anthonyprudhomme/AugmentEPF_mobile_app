...
public class PathPlanningActivity extends AppCompatActivity
    ...
    public static void expand(final View hideView) {
        // Get the measure of the view hide
        hideView.measure(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);

        // Final height of the new view
        final int targetHeight = hideView.getMeasuredHeight();

        hideView.getLayoutParams().height = 1; // To active the layout
        hideView.setVisibility(View.VISIBLE); // Set visible the layout
        Animation expandLayout = new Animation() // The animation to expand the layout
        {
            @Override
            protected void applyTransformation(float interpolatedTime, Transformation t) {
                hideView.getLayoutParams().height = interpolatedTime == 1
        ...
        // Duration of the expansion (1dp/ms)
        expandLayout.setDuration( (int)
                (targetHeight / hideView.getContext().getResources().getDisplayMetrics().density)
        );

        // Anim the hide view with the expansion
        hideView.startAnimation(expandLayout);
    }

    public static void collapse(final View visibleView) {
        // The height of the initial view
        final int initialHeight = visibleView.getMeasuredHeight();

        // Collapse the actual visible view
        Animation collapseLayout = new Animation()
        {
            @Override
            protected void applyTransformation(float interpolatedTime, Transformation t) {
                if (interpolatedTime == 1) {
                    ...
                    visibleView.getLayoutParams().height =
                            initialHeight - (int)(initialHeight * interpolatedTime);
                    visibleView.requestLayout();
                }
            }
        ...
        // Duration of the expansion (1dp/ms)
        collapseLayout.setDuration( (int)
                (initialHeight / visibleView.getContext().getResources().getDisplayMetrics().density)
        );

        // Anim the visible view with the collapsing
        visibleView.startAnimation(collapseLayout);
    }
